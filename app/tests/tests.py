...
async def model_loading_test(query_for_fandq:str|None = None, query_for_policy : str | None = None):
    ...
    try :
        if query_for_fandq :
            ...
            rag_result = await get_fandqs(query_for_fandq,embeddings, faiss_cache)
            print(f"results of the the RAG TOOL : { rag_result}")
        if query_for_policy :
            print("now running the policy tool")
            policy_result = await get_policy_recommendation(query_for_policy, embeddings, faiss_cache)
...
asyncio.run(model_loading_test(query_for_fandq=query_for_fandq, query_for_policy=query_for_policy))
